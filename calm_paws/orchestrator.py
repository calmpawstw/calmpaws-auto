...
def main():
    ...
    parser.add_argument("--duration-h", type=float, default=None,
                        help="覆寫本次 YouTube 長片時數（來自優化引擎的挑選）")
    ...
    # ── 覆寫本次長片時數 ──────────────────────────────────────────
    # 這裡曾經是個死角：cp_optimizer.py 的 Thompson Sampling 會挑
    # duration_h（1/2/3/8 小時...），cp_generate.py 也真的會嘗試把它
    # 傳進來（CLI 旗標、CP_DURATION_H 環境變數、scene_override.json
    # 三種管道都寫了），但這支檔案從來沒有讀取任何一種——實際編碼與
    # 標題文字用的都只是 config.yaml 裡那個寫死的
    # schedule.youtube_video_duration_hours，跟優化引擎選了什麼完全無關。
    # 結果就是不管優化引擎怎麼學，每支影片其實都用同一個固定時數，
    # 而那個固定值只要沒改對（例如卡在 8），就會整批卡在 YouTube
    # 處理中——這正是 8 小時長片一直卡住不會完成的根本原因。
    #
    # 修法：把解析出來的時數直接寫回 config["schedule"]，讓下游
    # （音樂/影片時長計算、YouTube 標題與說明欄的 {duration}）
    # 全部自動讀到正確值，不用逐一修改每個下游函式的簽名。
    dur_override = args.duration_h
    if dur_override is None:
        env_val = os.environ.get("CP_DURATION_H")
        if env_val:
            try:
                dur_override = float(env_val)
            except ValueError:
                pass
    if dur_override is None:
        override_file = BASE_DIR / "scene_override.json"
        if override_file.exists():
            try:
                ov = json.loads(override_file.read_text(encoding="utf-8"))
                if ov.get("duration_h"):
                    dur_override = float(ov["duration_h"])
            except Exception:
                pass
    if dur_override:
        config["schedule"]["youtube_video_duration_hours"] = dur_override

    logs_dir = Path(config["paths"]["logs_dir"])
    logger = setup_logging(logs_dir)
    logger.info(f"本次長片時數：{config['schedule']['youtube_video_duration_hours']} 小時"
                f"{'（優化引擎覆寫）' if dur_override else '（config.yaml 預設值）'}")
